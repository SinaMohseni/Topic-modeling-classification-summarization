...
def docs_summary(my_dictionary1, docs_number, ids2words, doc_vectors, output_topics, doc_topics, de_stemmer, class_num, keyword_num, filename, filename2, filename3):   # Print and save document topics/topic defenitions 
...
def topic_summary(my_dictionary1, docs_number, ids2words, doc_vectors, output_topics, doc_topics, de_stemmer, class_num, keyword_num, filename, filename2, filename3):  
...
def entity_summary(my_dictionary1, docs_number, ids2words, doc_vectors, output_topics, doc_topics, de_stemmer, class_num, keyword_num, filename, filename2, filename3):  
...
def save_features(data_set_docs, highlight_plus,search_terms, reading_weight, open_time, filename):  # doc number, topic mixture
    json_hash = []   # doc number, topic mixture
    count = 0
...
def save_notes(note_terms, saveNotes):  # Note number, topic mixture
    json_hash = []
    count = 0
...
magic_number = 20     # reading doc duration interaction weight
magic_number_2 = 5   #Doc_open duration intetaction weight
...
for class_num in xrange(3,4):

    for dataset_num in xrange(1,1):
        ...
        for participant_num in xrange(1,1): 
            ...
            doc_topic_array_, doc_key_word_ = save_topic_docs(my_dictionary, docs_number, ids2words_, doc_vectors_, output_topics_, doc_topics_,de_stemmer_, class_num,keyword_num, docTopicsFile, ldaTopicsFile, entityListFile)
            ...
            saveDocTopicModels = "./Dataset_"+str(dataset_num)+"/Dataset/Dataset_"+str(dataset_num)+"_features_P"+str(participant_num)+".json"
            saveNotes = "./Dataset_"+str(dataset_num)+"/Dataset/Notes_P"+str(participant_num)+".json"
            save_features(doc_topic_mixtures, saveDocTopicModels)
            save_notes(note_topic_mixtures, saveNotes)
